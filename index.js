...
activeOption2.addEventListener('click',()=>{switchTab(activeOption2)});
activeOption1.addEventListener('click',()=>{switchTab(activeOption1)});
...
function getFromsessionStorage() {
 ...
 async function fetchUserWeatherInfo(coordinates){
    ...
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`).then((response)=>response.json());
...
async function fetchSearchWeatherInfo(cityName){
    let response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${API_KEY}&units=metric`).then((response)=>response.json());
    weatherBox.classList.add("active");
    render(response);
}